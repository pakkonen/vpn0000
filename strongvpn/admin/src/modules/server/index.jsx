...
const Server = ({ global: { user } }) => {
  ...
  const [searchValue, setSearch] = React.useState({search_by: "country", search: ""})
  ...
  const remove = (row) => {
    POST('/admin/deleteServer', { id: row?.id, adminId: user?.id })
      .then(() => {
        notification.info({
        ...
        <div className="flex justify-between">
          ...
          <div className="flex">
            <UIButton className="secondary mr-3" onClick={() => showModalAddNewServer("PREMIUM")}>New premium
              server</UIButton>
            <UIButton className="third" onClick={() => showModalAddNewServer("FREE")}>New free server</UIButton>
          </div>
        </div>
              ...
              country: ({text, row: {countryCode}}) => (
                <div className="text-left flex items-center">
                  <img className="rounded-full mr-2" style={{border: "2px solid #f5f5f5"}}
                       src={`/flags/${countryCode.toLowerCase()}.svg`} width={24} height={24}/>
                  <span>{text}</span>
                </div>
              ),
              action: ({row}) => (
                <div className="flex items-center justify-center">
                  <Dropdown
                    ...
                    overlay={(user?.email && user?.email?.toLowerCase() !== EMAIL) ? (
